...
router.post('/progress/upsert', auth, async (req, res) => {
  try {
    ...
    // Calculate completed status (>=90% watched) or honor explicit isComplete true
    const completed = !!(isComplete || (duration > 0 && position / duration >= 0.9));

    // Get existing progress to calculate delta for secondsWatched
    const existing = await UserProgress.findOne({ userId, courseId, lectureId });
    const oldPosition = existing ? existing.position : 0;
    const delta = Math.max(0, position - oldPosition);
...
router.get('/progress/course/:courseId', auth, async (req, res) => {
  try {
    ...
    const totalWatched = items.reduce((sum, it) => sum + Math.min(it.position, it.duration), 0);
...
router.get('/profile/summary', auth, async (req, res) => {
  try {
    ...
    const coursesEnrolled = new Set(progress.map((p) => p.courseId.toString())).size;
    ...
    res.json({ coursesEnrolled, totalWatchTime, completedLectures });
  } catch (e) {
    res.status(500).json({ error: 'Server error' });
  }
});
